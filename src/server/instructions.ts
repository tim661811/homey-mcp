...
export function buildServerInstructions(options: BuildServerInstructionsOptions): string {
  ...
  sections.push(
    [
      ...
      '- "log()" output is NOT visible here. It goes to the HomeyScript app console, and nothing in this server can read it, so do not ask the user to paste it back. Return what needs checking instead: the run answers with the return value, so a script under development should return an object holding the intermediate values and be trimmed to the real answer once it works.',
